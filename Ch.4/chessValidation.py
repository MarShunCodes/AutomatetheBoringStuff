# Write your code here :-)
ChessBoard = {'1a': 'bking','2a': 'bqueen','3a': 'brook','4a': 'brook',
'5a': 'bknight','6a': 'bknight','7a':'bbishop','8a': 'bbishop',
'1b': 'bpawn','2b': 'bpawn','3b': 'bpawn','4b':'bpawn',
'5b': 'bpawn','6b': 'bpawn','7b': 'bpawn','8b': 'bpawn',
'1c': 'wking','2c': 'wqueen','3c': 'wrook','4c': 'wrook',
'5c': 'wbishop','6c': 'wbishop','7c': 'wknight','8c':'wknight',
'1e': 'wpawn','2e': 'wpawn','3e': 'wpawn','4e': 'wpawn',
'5e': 'wpawn','6e': 'wpawn','7e': 'wpawn','8e': 'wpawn',
'1f': '','2f': '','3f': '','4f': '','5f': '','6f': '','7f': '','8f': '',
'1g': '','2g': '','3g': '','4g': '','5g': '','6g': '','7g': '','8g': '',
'1h': '','2h': '','3h': '','4h': '','5h': '','6h': '','7h': '','8h': '',}

def isValidChessboard(ChessBoard):
    Pieces={'w':0, 'b':0}
    Pawns ={'wpawn':0,'bpawn':0}
    letterAxis = ('a','b','c','d','e','f','g','h')
    if 'bking' not in ChessBoard.values() or 'wking' not in ChessBoard.values() :
        print('problem with kings')
        return False
    for pos, p in ChessBoard.items():
        if p!= '':
            Pieces[p[0]]=Pieces[p[0]]+1
            if Pieces[p[0]] >= 17:
                print('problem with the number of pieces')
                return
        if p in Pawns.keys():
            Pawns[p]=Pawns[p] + 1

        if int(pos[0]) >=9 or pos[1] not in letterAxis :
            print('Wrongmove')
            return

    if Pawns['wpawn']>= 9 or Pawns['bpawn']>=9 :
        print('problem with pawns')
        return
    else:
        return 'Board is good'

print('Is Board Valid?')
print(isValidChessboard(ChessBoard))
